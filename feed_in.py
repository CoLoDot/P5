...
class Product:
	...
	def get_products_from_OFF_orangejuice(self): # method to get data with requests' module
		try:	
			...
			print("Un problème est survenu lors de la "+
				  "récupération du produit 'Jus d\'orange'.")
	...
	def get_products_from_OFF_chocolatespread(self): # method to get data with requests' module
		try:
			for self.page in range(0, TOTAL_PAGE_NUMBER_2):
				self.rpage = requests.get('https://fr.openfoodfacts.org/'+
											'categorie/pates-a-tartiner-au-chocolat/' 
											+ str(self.page+1) + '.json')
			...
			print("Un problème est survenu lors de la récupération"+
				  " du produit 'Pâte à tartiner au chocolat'.")
	...
	def get_products_from_OFF_toast(self): # method to get data with requests' module
		try:	
			...
			print("Un problème est survenu lors de la récupération du produit 'Biscottes'.")
	...
	def send_products_to_db(self): # method to send products to database
		try:	
			...
			cursor.execute("INSERT INTO `Category` (`id`, `Produit`) "
									...
									"(NULL, 'Biscottes')")
			cursor.executemany("INSERT INTO Product(id, produit_id, nom, marque, nutriscore, url) "
										"VALUES (NULL, %s, %s, %s, %s, %s)", data)
			cnx.commit()
		except:
			print("Un problème est survenu lors de l'insertion des données dans la BDD.")
